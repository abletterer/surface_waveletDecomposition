...
MapHandlerGen* Surface_WaveletDecomposition_Plugin::drawCoarseImage(const QString& mapName)
{
    if(m_decomposition)
    {
        ...
        int image_parentX = image_parent.width(), image_parentY = image.height();

        float width_step_parent = (image_parentX-1)/qAbs(max.x-min.x);
        float height_step_parent = (image_parentY-1)/qAbs(max.y-min.y);
